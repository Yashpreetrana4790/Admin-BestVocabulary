...
interface WordRelationsTabProps {
  wordId: string;
  currentWord: string;
  initialSynonyms?: RelationItem[];
  initialAntonyms?: RelationItem[];
  onSave?: (synonyms: RelationItem[], antonyms: RelationItem[]) => void;
}
        ...
        <div className="flex flex-col items-center justify-center h-full py-8 text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
            {isOver 
              ...
              : `No ${relationType}s added yet.`}
          </p>
          <p className="text-xs text-gray-400 dark:text-gray-500">
            Drag words from the sidebar to add them as {relationType}s for &quot;{currentWord}&quot;
          </p>
        </div>
      )}
    </div>
...
export const WordRelationsTab: React.FC<WordRelationsTabProps> = ({
  wordId,
  currentWord,
  initialSynonyms = [],
  initialAntonyms = [],
  ...
  const [synonyms, setSynonyms] = useState<RelationItem[]>(initialSynonyms);
  const [antonyms, setAntonyms] = useState<RelationItem[]>(initialAntonyms);
  ...
  useEffect(() => {
    ...
    setAntonyms(initialAntonyms || []);
  }, [initialSynonyms, initialAntonyms]);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active?.data?.current?.type === 'search-result' && over?.id) {
      ...
      if (targetBox === 'synonyms-box' || targetBox === 'antonyms-box') {
        ...
        // Don't allow adding the same word as a relation
        if (word?._id === wordId) {
          toast.error(`Cannot add "${word?.word}" as a relation to itself`);
          setActiveId(null);
          setDraggedWord(null);
          return;
        }
    ...
    if (over && active?.id !== over?.id && (active?.data?.current?.type === 'synonym' || active?.data?.current?.type === 'antonym')) {
      ...
      const currentRelations = relationType === 'synonym' ? synonyms : antonyms;
      const setRelations = relationType === 'synonym' ? setSynonyms : setAntonyms;
      ...
      if (oldIndex !== -1 && newIndex !== -1 && currentRelations) {
        setRelations(arrayMove(currentRelations, oldIndex, newIndex));
      }
    }
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      // Filter out words that are already added as synonyms or antonyms, and the current word
      const existingWordIds = [
        wordId, // Don't show the current word
  ...
  const handleAddRelation = (word: WordSearchResult, relationType: 'synonym' | 'antonym') => {
    ...
    if (relationType === 'synonym') {
      const updatedSynonyms = [...(synonyms || []), newRelation];
      setSynonyms(updatedSynonyms);
      toast.success(`Added "${word.word}" as a synonym`);
      if (onSave) {
        onSave(updatedSynonyms, antonyms || []);
      }
    } else {
      const updatedAntonyms = [...(antonyms || []), newRelation];
      setAntonyms(updatedAntonyms);
      toast.success(`Added "${word.word}" as an antonym`);
      if (onSave) {
        onSave(synonyms || [], updatedAntonyms);
      }
    }
  };

  const handleRemoveRelation = (wordIdToRemove: string, relationType: 'synonym' | 'antonym') => {
    if (!wordIdToRemove) return;
    
    if (relationType === 'synonym') {
      const updatedSynonyms = (synonyms || []).filter((s) => s?.wordId !== wordIdToRemove);
      setSynonyms(updatedSynonyms);
      ...
      const updatedAntonyms = (antonyms || []).filter((a) => a?.wordId !== wordIdToRemove);
      setAntonyms(updatedAntonyms);
          ...
          <Card className="h-full flex flex-col">
            <CardContent className="p-4 flex flex-col h-full">
              ...
              <div className="mb-4 p-3 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-950/30 dark:to-indigo-950/30 rounded-lg border border-purple-200 dark:border-purple-800">
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Managing relations for:</p>
                <h3 className="font-bold text-lg text-gray-900 dark:text-gray-100 capitalize">
                  {currentWord || 'Word'}
                </h3>
              </div>
            ...
            <CardContent className="p-4 h-full flex flex-col">
              <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'synonyms' | 'antonyms')} className="h-full flex flex-col">
                ...
                <TabsContent value="synonyms" className="flex-1 flex flex-col mt-0">
                  <div className="mb-4">
                    <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                      Synonyms for &quot;{currentWord}&quot;
                    </h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Drag words from the sidebar to add synonyms
                    </p>
                  </div>
                  <div className="flex-1 overflow-y-auto">
                    <DroppableRelationsBox
                      relations={synonyms || []}
                      onRemove={(wordIdToRemove) => handleRemoveRelation(wordIdToRemove, 'synonym')}
                ...
                <TabsContent value="antonyms" className="flex-1 flex flex-col mt-0">
                  <div className="mb-4">
                    <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                      Antonyms for &quot;{currentWord}&quot;
                    </h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Drag words from the sidebar to add antonyms
                    </p>
                  </div>
                  <div className="flex-1 overflow-y-auto">
                    <DroppableRelationsBox
                      relations={antonyms || []}
                      onRemove={(wordIdToRemove) => handleRemoveRelation(wordIdToRemove, 'antonym')}
