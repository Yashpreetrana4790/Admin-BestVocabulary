...
import { X, GripVertical, Search, Plus, Loader2, BookOpen } from 'lucide-react';
...
interface SynonymManagerProps {
  ...
  onChange: (synonyms: SynonymItem[]) => void;
  label?: string;
}

const SortableSynonymItem = ({
  item,
  onRemove,
}: {
  item: SynonymItem;
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center gap-3 p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg group hover:border-purple-300 dark:hover:border-purple-700 transition-all ${isDragging ? 'shadow-lg z-50' : ''
        }`}
    >
      <div
        {...attributes}
...
const DraggableSearchCard = ({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`relative group rounded-lg border-2 ${isAlreadyAdded
...
// Droppable Synonyms Box Component
const DroppableSynonymsBox = ({
  synonyms,
  onRemove,
}: {
  synonyms: SynonymItem[];
  ...
  return (
    <div
      ref={setDroppableRef}
      className={`min-h-[150px] rounded-lg border-2 border-dashed p-4 transition-colors ${isOver
    ...
    >
      {synonyms && synonyms.length > 0 ? (
        <SortableContext
          items={synonyms.map((s) => s?.wordId).filter(Boolean) as string[]}
          strategy={verticalListSortingStrategy}
        >
          <div className="space-y-2">
            {synonyms.map((synonym) => (
              <SortableSynonymItem
                key={synonym?.wordId || Math.random()}
                item={synonym}
                onRemove={() => onRemove(synonym?.wordId || '')}
              />
            ))}
          </div>
        </SortableContext>
        ...
        <div className="flex flex-col items-center justify-center h-full py-8 text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {isOver ? 'Drop word here to add as synonym' : 'No synonyms added yet. Drag words from search results to add them.'}
          </p>
        </div>
      )}
    </div>
...
export const SynonymManager: React.FC<SynonymManagerProps> = ({
  synonyms,
  onChange,
  label = 'Synonyms',
}) => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Handle dropping search result into synonyms box
    if (active?.data?.current?.type === 'search-result' && over?.id === 'synonyms-box') {
      const word = active?.data?.current?.word as WordSearchResult;
      const isAlreadyAdded = synonyms?.some((s) => s?.wordId === word?._id) ?? false;
      if (!isAlreadyAdded && word) {
        handleAddSynonym(word);
      } else if (word?.word) {
        toast.info(`"${word.word}" is already a synonym`);
      }
      setActiveId(null);
      setDraggedWord(null);
      return;
    ...
    // Handle reordering existing synonyms
    if (over && active?.id !== over?.id && active?.data?.current?.type === 'synonym') {
      const oldIndex = synonyms?.findIndex((s) => s?.wordId === active?.id) ?? -1;
      const newIndex = synonyms?.findIndex((s) => s?.wordId === over?.id) ?? -1;
      if (oldIndex !== -1 && newIndex !== -1) {
        onChange(arrayMove(synonyms, oldIndex, newIndex));
      }
    }
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      // Filter out words that are already synonyms
      const existingWordIds = synonyms?.map((s) => s?.wordId).filter(Boolean) ?? [];
  ...
  const handleAddSynonym = (word: WordSearchResult) => {
  ...
  const handleRemoveSynonym = (wordId: string) => {
    if (!wordId) return;
    onChange((synonyms || []).filter((s) => s?.wordId !== wordId));
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="text-xs">
              {synonyms?.length || 0} {(synonyms?.length || 0) === 1 ? 'synonym' : 'synonyms'}
            </Badge>
          </div>
          <Button
            variant="outline"
                    ...
                    {searchResults && searchResults.length > 0 ? (
                      searchResults.map((result) => (
                        <DraggableSearchCard
                          ...
                          word={result}
                          isAlreadyAdded={(synonyms || []).some((s) => s?.wordId === result?._id)}
                          onAdd={() => handleAddSynonym(result)}
                        />
                      ))
                      ...
                      <div className="flex flex-col items-center justify-center py-8 text-center">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Search for words to add as synonyms
                        </p>
          ...
          <div className="flex-1 min-w-0">
            <Card className="h-full">
              <CardContent className="p-4 h-full flex flex-col">
                ...
                {/* Droppable Synonyms Box */}
                <div className="flex-1 overflow-y-auto">
                  <DroppableSynonymsBox
                    synonyms={synonyms}
